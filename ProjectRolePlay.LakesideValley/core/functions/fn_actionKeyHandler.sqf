...
if((_curTarget getVariable["dead",FALSE]) && playerSide != independent && !dialog && _curTarget isKindOf "Man") exitwith {
	if(side _curTarget == independent || side _curTarget == west || side _curTarget == civilian) then {
...
if(isPlayer _curTarget && _curTarget isKindOf "Man") then {
	...
	if((_curTarget getVariable["ziptied",false]) && {!(player getVariable["ziptied",false])} && !dialog && playerSide == west) then 	{
			[_curTarget] call life_fnc_copInteractionMenu;	};
	if((_curTarget getVariable["restrained",false]) && {!(player getVariable["restrained",false])} && !dialog && playerSide == civilian) then	{
				    [_curTarget] call life_fnc_civInteractionMenu;	};
	if((_curTarget getVariable["ziptied",false]) && {!(player getVariable["ziptied",false])} && !dialog && playerSide == civilian) exitWith	{
				    [_curTarget] call life_fnc_civInteractionMenu;	};
		} else {
